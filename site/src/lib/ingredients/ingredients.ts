...
export abstract class IngredientControl<P> implements IIngredientControl<P> {
	...
	abstract attach(pane: Pane, params: P, store: Writable<P>): IngredientControlHandle;
}
...
export function updateIngredient(type: string, control: IngredientControl<any>) {
	ingredientsStore.update(($ingredients) => {
		$ingredients[type] = control;
		return $ingredients;
	});
}
export function deleteIngredient(type: string, control: IngredientControl<any>) {
	ingredientsStore.update(($ingredients) => {
